        ...
        <div className="center">
            ...
            <div id='poruciVino'>
                <form >
                    ...
                    <select name="vina" className='razmak' onChange={(e) => { setVina(e.target.value) }} >
                        {items.map((item) => (
                            <option key={item.id} >{item.title}</option>
                        ))}

                    </select>
                    <button onClick={(e) => { e.preventDefault(); posalji() }} className='razmak'>Poruči</button>
                </form>
            </div>
            <h6>Naručeno:</h6>
            <div id="prikaz">
